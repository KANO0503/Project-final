...
# 일본 지역 목록
japan_regions = [
    "신주쿠", "하라주쿠", "도쿄 디즈니랜드", "시부야 스크램블 교차로", 
    "긴자", "도쿄 타워", "스카이트리", "아사쿠사와 센소지", "메이지 신궁", 
    "우에노 공원", "롯폰기 힐즈", "시모키타자와"
]
...
if st.button("전송"):
    ...
    query = f"\"{menu_query}\" {destination} gnavi.co.jp 사이트의 종합적인 내용을 기반으로 일본 지역에 위치한 현재 영업중인 음식점 가게의 리뷰, 상세 정보와 가게 정보(주소,전화번호,영업시간,가격대)와 별 점수가 5점에 가까운 랭킹 1위~5위 함께 알려주세요"
    
    # "로컬 찐 맛집을 찾고 있어요. 조금만 기다려주세요" 문구 출력가

...
if st.button("전송"):
    ...
    query = f"\"{menu_query}\" {destination} tabelog.com 사이트를 기반으로 입력된 도쿄 지역 유명 명소 근처에 위치한 현재 영업중이고 별 점수가 5점 만점에 가까운 랭킹 1위~5위 맛집, 가게 리뷰, 가게 정보(가게명은 일본어로, 주소, 전화번호, 영업시간, 가격대) 함께 추천해주세요"

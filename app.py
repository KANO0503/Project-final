...
if st.button("전송"):
    ...
    query = f"\"{menu_query}\" {destination} tabelog.com/tokyo 사이트의 내용을 기반으로 사용자가 입력한 도쿄 지역에 유명 명소 근처에 위치한 현재 영업중이고 별 점수가 5점에 가까운 랭킹 1위~5위 맛집, 가게 리뷰, 가게 정보(가게명은 일본어(한국어)로 둘다표기, 주소, 전화번호, 영업시간, 가격대) 함께 추천해주세요"
